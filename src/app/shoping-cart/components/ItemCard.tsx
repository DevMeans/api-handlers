...
import type { Product } from "@/data/propducts"
...
export const ItemCard = ({ product, quantity }: Props) => {
  ...
  function onAddToCart(id: string, valor: number) {
    //TODO: addProductToCart(product.id);
    removeSingleitemFromCart(id, valor)
    router.refresh();
  }

  function onRemoveItem(id: string, valor: number) {
  ...
  return (
    <div className="flex items-center shadow rounded-lg w-full bg-gray-800 border-gray-100">
      ...
      <div className="flex p-5 items-center justify-center">
        ...
        <button
          onClick={()=>onRemoveItem(product.id,-1)}
